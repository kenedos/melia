...
namespace Melia.Zone.Skills.Handlers.Archers.Ranger
{
	...
	public class Ranger_SpiralArrowOverride : IForceSkillHandler, IDynamicCasted
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, Position originPos, Position farPos, ICombatEntity target)
		{
			...
			SkillRepeatHelper.Request(skill, caster, target, hitTarget => this.SingleAttack(skill, caster, hitTarget), () => Send.ZC_SKILL_FORCE_TARGET(caster, target, skill, ForceId.GetNew(), null));
		}
		...
		/// <param name="caster"></param>
		/// <param name="target"></param>
		public bool SingleAttack(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
			{
				caster.ServerMessage(Localization.Get("Not enough SP."));
