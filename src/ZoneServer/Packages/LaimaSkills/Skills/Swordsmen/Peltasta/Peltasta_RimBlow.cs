...
namespace Melia.Zone.Skills.Handlers.Swordsmen.Peltasta
{
	...
	public class Peltasta_RimBlowOverride : IGroundSkillHandler
	{
		...
		private async Task Attack(Skill skill, ICombatEntity caster, ISplashArea splashArea)
		{
			...
			foreach (var target in targets.LimitBySDR(caster, skill))
			{
				...
				if (skillHitResult.Damage > 0
					&& caster.TryGetActiveAbilityLevel(AbilityId.Impact, out int stunLevel) && GameRandom.Get().Next(100) < stunLevel * StunChancePerLevel)
