...
namespace Melia.Zone.Packages.LaimaSkills.Pads.Swordsmen.Peltasta
{
	/// <summary>
	...
	public class Peltasta_ShieldLobOverride : ICreatePadHandler, IDestroyPadHandler
	{
		...
		public void Created(object sender, PadTriggerArgs args)
		{
			...
			var shieldMonster = new Mob(57001, RelationType.Friendly);
			...
			shieldMonster.Layer = creator.Layer;
			shieldMonster.AttachEffect("I_light004_violet", 1.5f);
			if (creator.TryGetEquipItem(EquipSlot.LeftHand, out var lhItem))
			{
				...
				shieldMonster.AddEffect(new SetModelEffect("warrior_f_", lhItem.Id));
			}
			shieldMonster.AddEffect(new RotateEffect(90, 0, 0));
			shieldMonster.AddEffect(new SpinObjectEffect(0, -1, 0.2f, 0));
			shieldMonster.AddEffect(new TranslationEffect(pad.Handle, 22));
