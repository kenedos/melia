...
namespace Melia.Zone.Packages.Laima.Skills.Wizards.RuneCaster
{
	...
	/// SkillId: 21304
	/// ClassName: RuneCaster_Tiwaz
	///
	/// Behavior:
	/// - MELEE_GROUND magic attack.
	/// - Hits enemies in front of the caster.
	/// - Deals 5 hits.
	/// - Has 2 overheats from skill.ies.
	/// - Supports RuneCaster5: Rune of Justice Enhance.
	/// </summary>
	[Package("laima")]
	...
	public class RuneCaster_TiwazOverride : IGroundSkillHandler, IMeleeGroundSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, Skill skill, IList<ICombatEntity> targets)
		{
			...
			foreach (var target in targets.Where(target => target != null && !target.IsDead))
			{
				for (var i = 0; i < HitCount; i++)
				{
					...
					this.ApplyRuneOfJusticeEnhance(caster, skillHitResult);
		...
		private void ApplyRuneOfJusticeEnhance(ICombatEntity caster, SkillHitResult skillHitResult)
		{
