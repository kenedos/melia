...
namespace Melia.Zone.Packages.Laima.Skills.Wizards.RuneCaster
{
	...
	/// SkillId: 21303
	/// ClassName: RuneCaster_Thurisaz
	///
	/// Behavior:
	/// - MELEE_GROUND magic attack.
	/// - Channeling skill for 2 seconds.
	/// - Deals damage every 0.2 seconds, total 10 hits.
	/// - Uses Psychokinesis/Soul property from skill.ies.
	/// - Supports RuneCaster23: Rune of Repulsion Enhance.
	/// </summary>
	[Package("laima")]
	...
	public class RuneCaster_ThurisazOverride : IGroundSkillHandler, IMeleeGroundSkillHandler
	{
		...
		private static readonly TimeSpan ChannelDuration = TimeSpan.FromSeconds(2);
		...
		private void Cast(Skill skill, ICombatEntity caster, Position originPos, Position farPos, IList<ICombatEntity> targets)
		{
			...
			skill.Run(this.HandleChannel(caster, skill, validTargets));
		}

		private async Task HandleChannel(ICombatEntity caster, Skill skill, IList<ICombatEntity> targets)
		{
			var endTime = DateTime.Now.Add(ChannelDuration);
			var hitIndex = 0;
			...
			while (DateTime.Now < endTime && hitIndex < HitCount)
			{
				foreach (var target in targets.Where(target => target != null && !target.IsDead))
				{
					...
					this.ApplyRuneOfRepulsionEnhance(caster, skillHitResult);
		...
		private void ApplyRuneOfRepulsionEnhance(ICombatEntity caster, SkillHitResult skillHitResult)
		{
