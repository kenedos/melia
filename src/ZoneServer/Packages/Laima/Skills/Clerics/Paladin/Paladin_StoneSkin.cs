...
{
	/// <summary>
	/// Handler for the Paladin skill Stone Skin.
	/// </summary>
	[Package("laima")]
	[SkillHandler(SkillId.Paladin_StoneSkin)]
	public class Paladin_StoneSkinOverride : IMeleeGroundSkillHandler, IDynamicCasted
	{
		...
		private async Task HandleSkill(ICombatEntity caster, Skill skill)
		{
			...
			caster.StartBuff(BuffId.StoneSkin_Buff, skill.Level, 0f, TimeSpan.FromSeconds(BuffDurationSeconds), caster);
			...
			if (caster is Character character)
			{
				...
				if (party != null)
				{
					foreach (var member in members)
					{
						if (member == caster)
							continue;
						member.StartBuff(BuffId.StoneSkin_Buff, skill.Level, 0f, TimeSpan.FromSeconds(BuffDurationSeconds), caster);
					}
				}
			}
		}
