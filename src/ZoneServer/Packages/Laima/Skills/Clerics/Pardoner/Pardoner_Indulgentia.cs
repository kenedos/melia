...
namespace Melia.Zone.Skills.Handlers.Clerics.Pardoner
{
	...
	/// Applies the Indulgentia buff to nearby party members and cancels their
	/// debuffs. While the buff is active, targets recover HP in regular intervals.
	/// The buff's effectiveness is doubled when the skill is cast from a scroll.
	/// </summary>
	[Package("laima")]
	[SkillHandler(SkillId.Pardoner_Indulgentia)]
	public class Pardoner_IndulgentiaOverride : IGroundSkillHandler, IDynamicCasted
	{
		private const float TargetRange = 90f;
		private const float ScrollEffectiveness = 2f;
		...
		private async Task HandleSkill(ICombatEntity caster, Skill skill)
		{
			...
			var effectiveness = skill.IsItemSkill ? ScrollEffectiveness : 1f;
			...
			foreach (var target in skillTargets)
				target.StartBuff(BuffId.Indulgentia_Buff, skill.Level, effectiveness, buffDuration, caster, skill.Id);
