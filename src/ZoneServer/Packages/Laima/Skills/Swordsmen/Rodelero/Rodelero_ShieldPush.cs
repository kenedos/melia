...
namespace Melia.Zone.Skills.Handlers.Rodelero
{
	...
	/// Multi-hit shield attack that pushes enemies back and reduces their defense.
	/// </summary>
	[Package("laima")]
	[SkillHandler(SkillId.Rodelero_ShieldPush)]
	public class Rodelero_ShieldPushOverride : IMeleeGroundSkillHandler
	{
		/// <summary>
		/// Handles the Shield Push skill execution.
		/// </summary>
		...
		/// <summary>
		/// Executes the attack with pushback and unbalance.
		/// </summary>
		private async Task Attack(Skill skill, ICombatEntity caster, Position originPos, Position farPos)
		{
			...
			foreach (var target in targetList.LimitBySDR(caster, skill))
			{
				...
				if (skillHitResult.Damage > 0)
				{
					...
					{
						skillHit.KnockBackInfo = new KnockBackInfo(caster, target, KnockBackType.KnockBack, 150, 10, KnockDirection.CasterForward);
						skillHit.HitInfo.KnockBackType = KnockBackType.KnockBack;
						target.ApplyKnockback(caster, skill, skillHit);
					}

					target.StartBuff(BuffId.ShieldPush_Debuff, skill.Level, 0f, TimeSpan.FromMilliseconds(3000), caster);
				}
