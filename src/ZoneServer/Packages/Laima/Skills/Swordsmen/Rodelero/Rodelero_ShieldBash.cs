...
{
	/// <summary>
	/// Handler for the Rodelero skill Shield Bash.
	/// Multi-hit AoE shield attack that breaks enemy armor.
	/// </summary>
	[Package("laima")]
	[SkillHandler(SkillId.Rodelero_ShieldBash)]
	public class Rodelero_ShieldBashOverride : IMeleeGroundSkillHandler
	{
		/// <summary>
		/// Handles the Shield Bash skill execution.
		/// </summary>
		...
		/// <summary>
		/// Executes the multi-hit attack with armor break debuff.
		/// </summary>
		private async Task Attack(Skill skill, ICombatEntity caster, Position originPos, Position farPos)
		{
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 55, width: 40, angle: 15f);
			...
			foreach (var target in targetList.LimitBySDR(caster, skill))
			{
				...
				target.StartBuff(BuffId.ShieldBash_Debuff, skill.Level, 0f, TimeSpan.FromMilliseconds(10000), caster);
			}
