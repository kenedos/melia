...
{
	/// <summary>
	/// Handler for Nak Muay skill Te Trong.
	/// Requires Ram Muay stance.
	/// </summary>
	[Package("laima")]
	[SkillHandler(SkillId.NakMuay_TeTrong)]
	public class NakMuay_TeTrongOverride : ITargetSkillHandler, IGroundSkillHandler, IMeleeGroundSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, Skill skill, IList<ICombatEntity> targets)
		{
			...
			foreach (var target in targets)
			{
				...
				if (caster is Character character && character.Abilities.TryGet(AbilityId.NakMuay3, out var ability) &&	ability.Active)
				{
