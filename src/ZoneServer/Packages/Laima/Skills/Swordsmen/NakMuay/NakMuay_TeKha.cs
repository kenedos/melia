...
{
	/// <summary>
	/// Handler for Nak Muay skill Te Kha.
	/// Requires Ram Muay stance.
	/// </summary>
	[Package("laima")]
	[SkillHandler(SkillId.NakMuay_TeKha)]
	public class NakMuay_TeKhaOverride : ITargetSkillHandler, IGroundSkillHandler, IMeleeGroundSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, Skill skill, IList<ICombatEntity> targets)
		{
			...
			foreach (var target in targets)
			{
				...
				if (caster is Character character && character.Abilities.TryGet(AbilityId.NakMuay1, out var ability) &&	ability.Active)
				{
