...
{
	/// <summary>
	/// Handler for Nak Muay skill Khao Loi.
	/// Requires Ram Muay stance.
	/// </summary>
	[Package("laima")]
	[SkillHandler(SkillId.NakMuay_KhaoLoi)]
	public class NakMuay_KhaoLoiOverride : ITargetSkillHandler, IGroundSkillHandler, IMeleeGroundSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, Skill skill, IList<ICombatEntity> targets)
		{
			...
			foreach (var target in targets)
			{
				...
				if (caster is Character character && character.Abilities.TryGet(AbilityId.NakMuay4, out var ability) &&	ability.Active)
				{
