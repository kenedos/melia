...
{
	/// <summary>
	/// Handler override for the Hoplite Skill Synchro Thrusting.
	/// </summary>
	[Package("laima")]
	...
	public class Hoplite_SynchroThrustingOverride : IMeleeGroundSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, Position originPos, Position farPos, params ICombatEntity[] targets)
		{
			...
			if (caster is Character casterCharacter)
			{
				...
				if (weapon == null || weapon.Data.EquipType1 != EquipType.Spear)
				{
					casterCharacter.ServerMessage(Localization.Get("Skill requires a one-handed spear."));
			...
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 40, width: 25, angle: 0);
			...
			if (caster is Character character)
			{
				...
				if (lhItem.Data.EquipType1 == EquipType.Shield)
					character.StartBuff(BuffId.Skill_MomentaryBlock_Buff, skill.Level, 0, BlockDuration, caster);
			}
