...
namespace Melia.Zone.Skills.Handlers.Archers.Fletcher
{
	...
	public class Fletcher_BodkinPointOverride : IPassiveSkillHandler
	{
		...
		{
			if (!caster.TryGetSkill(SkillId.Fletcher_BodkinPoint, out _))
				return;

			if (caster.TryGetActiveAbility(AbilityId.Fletcher43, out _))
				return;

			if (!Fletcher_FletcherArrowShotOverride.HasQuiverSpace(caster, Fletcher_FletcherArrowShotOverride.BodkinCost))
				return;
