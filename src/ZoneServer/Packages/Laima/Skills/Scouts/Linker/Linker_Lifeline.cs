...
{
	/// <summary>
	/// Handler for the Linker skill Lifeline (Umbilical Cord).
	/// </summary>
	[Package("laima")]
	[SkillHandler(SkillId.Linker_UmbilicalCord)]
	public class Linker_LifelineOverride : IMeleeGroundSkillHandler
	{
		private const float LinkRange = 250f;
		private const string LinkTexture = "Linker2";
		...
		{
			var buffsToDestroy = new[] { BuffId.Link_Sacrifice };
		...
		private void CreatePartyLink(ICombatEntity caster, Skill skill)
		{
			...
			for (var i = 0; i < linkedEntities.Count; i++)
			{
				...
				var buff = entity.StartBuff(BuffId.Link_Sacrifice, skill.Level, 0, TimeSpan.FromMinutes(5), caster, skill.Id);
				...
				if (buff != null)
				{
					...
					if (i == 0)
					{
						...
						entity.AddEffect("Melia.Link.Chain", new AttachEffect("I_chain004_mash_loop_multi_red", 2, EffectLocation.Bottom));
					}
				}
			}
