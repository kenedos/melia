...
namespace Melia.Zone.Skills.Handlers.Scouts.Assassin
{
	...
	public class Assassin_InstantaneousAccelerationOverride : IMeleeGroundSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, Position originPos, Position farPos, ICombatEntity[] targets)
		{
			...
			var endingPosition = caster.Position.GetRelative(caster.Direction, 100f);
			endingPosition = caster.Map.Ground.GetLastValidPosition(caster.Position, endingPosition);

			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 80f, width: 30, angle: 0);
