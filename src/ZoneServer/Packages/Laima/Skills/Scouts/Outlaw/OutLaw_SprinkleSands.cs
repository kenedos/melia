...
namespace Melia.Zone.Skills.Handlers.Scouts.OutLaw
{
	...
	public class OutLaw_SprinkleSandsOverride : IMeleeGroundSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, Position originPos, Position farPos, ICombatEntity[] designatedTargets)
		{
			...
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 90, width: 50, angle: 0);
		...
		private async Task Attack(Skill skill, ICombatEntity caster, ISplashArea splashArea, Position farPos)
		{
			...
			foreach (var target in hitTargets)
			{
				...
				{
					target.StartBuff(BuffId.SprinkleSands_Debuff, skill.Level, 0, TimeSpan.FromSeconds(15), caster);
				}
			}
