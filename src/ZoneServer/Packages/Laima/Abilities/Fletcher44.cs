...
{
	[Package("laima")]
	[AbilityHandler(AbilityId.Fletcher44)]
	public class Fletcher44Override : IAbilityPropertyHandler
	{
		...
		{
			if (!character.TryGetSkill(SkillId.Fletcher_BarbedArrow, out var skill) || skill.IsOnCooldown)
				return;

			if (!Fletcher_FletcherArrowShotOverride.HasQuiverSpace(character, Fletcher_FletcherArrowShotOverride.BarbedCost))
				return;
			...
			character.StartBuff(BuffId.Fletcher_BarbedArrow_Buff, TimeSpan.Zero);
		}
	}
}
