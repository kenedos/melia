...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_Trap_Desmodus_Skill_1 : ITargetSkillHandler
	{
	...
	public class Mon_Trap_ellomago_Skill_1 : ITargetSkillHandler
	{
		protected TimeSpan DamageDelay { get; } = TimeSpan.Zero;
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 30, width: 30, angle: 25f);
			var splashArea = skill.GetSplashArea(SplashType.Circle, splashParam);
			var hitDelay = 800;
			var damageDelay = 0;
			await SkillAttack(caster, skill, splashArea, hitDelay, damageDelay);
