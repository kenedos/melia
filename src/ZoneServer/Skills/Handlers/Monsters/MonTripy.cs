...
{
	[SkillHandler(SkillId.Mon_Tripy_Skill_1)]
	public class Mon_Tripy_Skill_1 : ITargetSkillHandler
	{
	...
	[SkillHandler(SkillId.Mon_Tripy_Skill_2)]
	public class Mon_Tripy_Skill_2 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			{
				Effect = new EffectConfig("I_force036_green#Dummy_Spine", 0.5f),
				EndEffect = new EffectConfig("F_explosion071_green2", 0.5f),
				Range = 15f,
				FlyTime = 0.5f,
				DelayTime = 0f,
				Gravity = 150f,
				...
				HitTime = 1000f,
				HitCount = 1,
				GroundEffect = new EffectConfig("F_sys_target_monster##0.5", 0.3f),
			});
		}
	}
