...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_ellogua_Skill_1 : ITargetSkillHandler
	{
	...
	[SkillHandler(SkillId.Mon_ellogua_Skill_2)]
	public class Mon_ellogua_Skill_2 : ITargetSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			...
			var originPos = caster.Position;
			var hitDelay = 0 + (int)(caster.Position.Get2DDistance(target.Position) * 7.3);
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos, int hitDelay, int aniTime)
		{
			...
			{
				Effect = new EffectConfig("I_smoke037_yellow#B_leaf3", 1f),
				EndEffect = new EffectConfig("F_ground011_yellow", 1f),
				Range = 15f,
				FlyTime = 0.75f,
				DelayTime = 0f,
				Gravity = 0f,
