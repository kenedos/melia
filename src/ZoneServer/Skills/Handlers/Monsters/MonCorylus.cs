...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_Corylus_Skill_1 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos, int hitDelay, int aniTime)
		{
			...
			SkillResultTargetBuff(caster, skill, BuffId.UC_silence, 1, 0f, 4000f, 1, 15, -1, hits);
		}
	}

	[SkillHandler(SkillId.Mon_Corylus_Skill_2)]
	public class Mon_Corylus_Skill_2 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			{
				GroundEffect = new EffectConfig("F_sys_target_monster##0.5", 0.8f),
				PositionDelay = 1000,
