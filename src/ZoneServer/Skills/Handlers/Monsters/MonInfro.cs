...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_infro_Blud_Skill_1 : ITargetSkillHandler
	{
	...
	[SkillHandler(SkillId.Mon_infro_Crabil_Skill_1)]
	public class Mon_infro_Crabil_Skill_1 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			var aniTime = 300;
			await SkillAttack(caster, skill, splashArea, hitDelay, aniTime);
			splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 45, width: 30, angle: 30f);
	...
	[SkillHandler(SkillId.Mon_infro_Crabil_Skill_2)]
	public class Mon_infro_Crabil_Skill_2 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			await EffectHitArrow(skill, caster, startingPosition, endingPosition, new ArrowConfig
			{
				...
				ArrowLifeTime = 0.1f,
				PositionDelay = 1000f,
				HitEffect = new EffectConfig("I_ground009_water_short2", 1f),
				Range = 15f,
				KnockdownPower = 100f,
				Delay = 0f,
