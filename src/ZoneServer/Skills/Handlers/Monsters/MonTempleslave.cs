...
{
	[SkillHandler(SkillId.Mon_Templeslave_mage_Skill_1)]
	public class Mon_Templeslave_mage_Skill_1 : ITargetSkillHandler
	{
	...
	public class Mon_Templeslave_mage_Skill_2 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			SkillCreatePad(caster, skill, farPos, 0f, PadName.Sage_HoleOfDarkness);
		}
	}

	[SkillHandler(SkillId.Mon_Templeslave_Skill_1)]
	public class Mon_Templeslave_Skill_1 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			var hitDelay = 800 + (int)(caster.Position.Get2DDistance(target.Position) * 1.8);
			var aniTime = hitDelay + 200;
			await SkillAttack(caster, skill, splashArea, hitDelay, aniTime);
		}
	}
	...
	public class Mon_Templeslave_sword_Skill_1 : ITargetSkillHandler
	{
		protected TimeSpan AniTime { get; } = TimeSpan.FromMilliseconds(1000);
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
	...
	public class Mon_Templeslave_sword_Skill_2 : ITargetSkillHandler
	{
		protected TimeSpan AniTime { get; } = TimeSpan.FromMilliseconds(1000);
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			var hitDelay = 800;
			var aniTime = 1000;
			if (caster.IsBuffActive(BuffId.Cloaking_Buff))
				caster.StopBuff(BuffId.Cloaking_Buff);
			await SkillAttack(caster, skill, splashArea, hitDelay, aniTime);
			caster.StartBuff(BuffId.Cloaking_Buff, 1, 0f, TimeSpan.FromSeconds(10), caster);
		}
	}
}
