...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_Pagclamper_Skill_1 : ITargetSkillHandler
	{
		...
		{
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 45, width: 25, angle: 25f);
			var splashArea = skill.GetSplashArea(SplashType.Circle, splashParam);
			await SkillAttack(caster, skill, splashArea, hitDelay, aniTime);
		}
	}

	[SkillHandler(SkillId.Mon_Pagclamper_Skill_2)]
	public class Mon_Pagclamper_Skill_2 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			await EffectAndHit(skill, caster, position, new EffectHitConfig
			{
				...
				PositionDelay = 0,
				Effect = new EffectConfig("F_ground058_smoke", 0.8f),
				Range = 25f,
				KnockdownPower = 120f,
				Delay = 0f,
				HitCount = 1,
