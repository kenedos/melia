...
{
	[SkillHandler(SkillId.Mon_wajak_walker_Skill_1)]
	public class Mon_wajak_walker_Skill_1 : ITargetSkillHandler
	{
	...
	public class Mon_wajak_walker_Skill_2 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 50, width: 10, angle: 40f);
			var splashArea = skill.GetSplashArea(SplashType.Fan, splashParam);
			var hitDelay = 600;
			var damageDelay = 800;
			await SkillAttack(caster, skill, splashArea, hitDelay, damageDelay);
