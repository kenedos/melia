...
namespace Melia.Zone.Skills.Handlers.Monsters.Boss
{
	...
	public class Mon_boss_Lithorex_Skill_1 : ITargetSkillHandler
	{
	...
	[SkillHandler(SkillId.Mon_boss_Lithorex_Skill_2)]
	public class Mon_boss_Lithorex_Skill_2 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			targetPos = GetRelativePosition(PosType.TargetRandomDistance, caster, target, rand: 130);
			SkillCreatePad(caster, skill, targetPos, 0f, PadName.Lithorex_bubble);
			targetPos = GetRelativePosition(PosType.TargetRandomDistance, caster, target, rand: 50);
			SkillCreatePad(caster, skill, targetPos, 0f, PadName.Lithorex_bubble);
			targetPos = GetRelativePosition(PosType.TargetRandomDistance, caster, target, rand: 130);
			SkillCreatePad(caster, skill, targetPos, 0f, PadName.Lithorex_bubble);
		}
	...
	public class Mon_boss_Lithorex_Skill_3 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
	...
	[SkillHandler(SkillId.Mon_boss_Lithorex_Skill_4)]
	public class Mon_boss_Lithorex_Skill_4 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			caster.StartBuff(BuffId.Magic_Shield, 1f, 0f, TimeSpan.FromMilliseconds(10000f), caster);
		}
	}
}
