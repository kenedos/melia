...
namespace Melia.Zone.Skills.Handlers.Monsters
{
	...
	public class Mon_boss_ShadowGaoler_Skill_1 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(Skill skill, ICombatEntity caster, Position originPos, Position farPos)
		{
			...
			var hitDelays = new[] { 0, 0 };
			var aniTimes = new[] { 1300, 400 };
	...
	public class Mon_boss_ShadowGaoler_Skill_3 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(Skill skill, ICombatEntity caster, ICombatEntity target, Position originPos, Position farPos)
		{
			...
			SkillResultTargetBuff(caster, skill, BuffId.UC_poison, 1, hits.Sum(h => h.HitInfo.Damage) * 0.2f, 8000f, 1, 40, -1, hits);
		}
	}
	...
	public class Mon_boss_ShadowGaoler_Skill_4 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(Skill skill, ICombatEntity caster, ICombatEntity target, Position originPos, Position farPos)
		{
			...
			var waits = new[] { 0, 300, 0, 300, 300, 300, 0, 2300, 200, 0, 200, 0, 200, 200, 2200, 200, 200, 0, 200, 200, 0 };
			var posTypes = new[] {
				PosType.TargetRandomDistance, PosType.TargetHeight, PosType.TargetHeight, PosType.TargetRandomDistance, PosType.TargetRandomDistance, PosType.TargetRandomDistance, PosType.TargetRandomDistance,
				PosType.TargetRandomDistance, PosType.TargetHeight, PosType.TargetHeight, PosType.TargetRandomDistance, PosType.TargetRandomDistance, PosType.TargetRandomDistance, PosType.TargetRandomDistance,
				PosType.TargetRandomDistance, PosType.TargetHeight, PosType.TargetRandomDistance, PosType.TargetRandomDistance, PosType.TargetRandomDistance, PosType.TargetRandomDistance, PosType.TargetRandomDistance
			};
			for (var i = 0; i < 21; i++)
			{
				if (waits[i] > 0)
				...
				_ = MissileThrow(skill, caster, position, config, hits);
			}
