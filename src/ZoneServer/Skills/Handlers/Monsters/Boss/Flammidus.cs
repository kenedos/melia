...
{
	[SkillHandler(SkillId.Mon_boss_Flammidus_Skill_1)]
	public class Mon_boss_Flammidus_Skill_1 : ITargetSkillHandler
	{
	...
	public class Mon_boss_Flammidus_Skill_2 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 70, width: 15, angle: 30f);
			var splashArea = skill.GetSplashArea(SplashType.Circle, splashParam);
			var hitDelay = 2100;
			var aniTime = 2300;
			await SkillAttack(caster, skill, splashArea, hitDelay, aniTime);
	...
	[SkillHandler(SkillId.Mon_boss_Flammidus_Skill_3)]
	public class Mon_boss_Flammidus_Skill_3 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			var config = new ArrowConfig
			{
				...
				ArrowLifeTime = 1f,
				PositionDelay = 0f,
				HitEffect = new EffectConfig("F_burstup030_fire_1", 0.3f),
				Range = 20f,
				KnockdownPower = 0f,
				Delay = 0f,
	...
	public class Mon_boss_Flammidus_Skill_4 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			await EffectAndHit(skill, caster, position, new EffectHitConfig
			{
				GroundEffect = new EffectConfig("F_spread_in007_dark#Bip001 L Calf", 1f),
				PositionDelay = 2000,
				Effect = new EffectConfig("F_spread_out026_leaf_fire", 1.3f),
				Range = 100f,
				KnockdownPower = 150f,
				Delay = 0f,
				HitCount = 1,
			...
			SkillResultTargetBuff(caster, skill, BuffId.UC_flame, 1, hits.Sum(h => h.HitInfo.Damage) * 0.5f, 10000f, 1, 50, -1, hits);
		}
	}

	[SkillHandler(SkillId.Mon_boss_Flammidus_Skill_5)]
	public class Mon_boss_Flammidus_Skill_5 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
