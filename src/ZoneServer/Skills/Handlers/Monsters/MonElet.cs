...
{
	[SkillHandler(SkillId.Mon_Elet_Skill_1)]
	public class Mon_Elet_Skill_1 : ITargetSkillHandler
	{
	...
	[SkillHandler(SkillId.Mon_Elet_Skill_2)]
	public class Mon_Elet_Skill_2 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			if (!await MonsterCastTime(skill, caster, "Blizzard", CastTimeMs, target))
				return;

			SkillCreatePad(caster, skill, target.Position, 0, PadName.Elementalist_Hail);
		}
	}
}
