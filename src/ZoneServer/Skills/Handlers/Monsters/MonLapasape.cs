...
{
	[SkillHandler(SkillId.Mon_lapasape_bow_Skill_1)]
	public class Mon_lapasape_bow_Skill_1 : ITargetSkillHandler
	{
	...
	[SkillHandler(SkillId.Mon_lapasape_mage_Skill_1)]
	public class Mon_lapasape_mage_Skill_1 : ITargetSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			...
			var originPos = caster.Position;
			var hitDelay = 1200 + (int)(caster.Position.Get2DDistance(target.Position) * 5.7);
			var aniTime = hitDelay + 200;
			var leadPos = GetLeadPosition(target, hitDelay, caster);
			caster.TurnTowards(leadPos);
			var farPos = originPos.GetNearestPositionWithinDistance(leadPos, skill.Properties[PropertyName.MaxR]);
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos, int hitDelay, int aniTime)
		{
			...
			SkillResultTargetBuff(caster, skill, BuffId.UC_poison, 1, hits.Sum(h => h.HitInfo.Damage) * 0.2f, 7000f, 1, 20, -1, hits);

		}
	}

	[SkillHandler(SkillId.Mon_Lapasape_Skill_1)]
	public class Mon_Lapasape_Skill_1 : ITargetSkillHandler
	{
