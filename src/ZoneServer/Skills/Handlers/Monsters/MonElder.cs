...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_elder_rode_Skill_1 : ITargetSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			...
			var hitDelay = 400 + (int)(caster.Position.Get2DDistance(target.Position) * 2.2);
		...
		{
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 120, width: 8, angle: 10f);
	...
	[SkillHandler(SkillId.Mon_elder_rode_Skill_2)]
	public class Mon_elder_rode_Skill_2 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			await EffectAndHit(skill, caster, position, new EffectHitConfig
			{
				...
				PositionDelay = 0,
				Effect = new EffectConfig("I_explosion004_yellow_mage01_3", 2f),
				Range = 40f,
				KnockdownPower = 150f,
				Delay = 0f,
				HitCount = 1,
			...
			SkillResultTargetBuff(caster, skill, BuffId.Mon_Magic_Shield_Debuff, 1, 0f, 10000f, 1, 100, -1, hits);
		}
	}
}
