...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_Tetraffid_Skill_1 : ITargetSkillHandler
	{
	...
	public class Mon_Tetraffid_Skill_2 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos, int hitDelay, int aniTime)
		{
			var splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 55, width: 30, angle: 50f);
			var splashArea = skill.GetSplashArea(SplashType.Circle, splashParam);
			var hits = new List<SkillHitInfo>();
			await SkillAttack(caster, skill, splashArea, hitDelay, aniTime, hits);
			SkillResultTargetBuff(caster, skill, BuffId.UC_silence_normalAttack, 1, 0f, 5000f, 1, 20, -1, hits);
		}
	}
