...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_Cronewt_bow_brown_Skill_1 : ITargetSkillHandler
	{
	...
	public class Mon_Cronewt_bow_Skill_1 : ITargetSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			...
			var originPos = caster.Position;
			var hitDelay = 300 + (int)(caster.Position.Get2DDistance(target.Position) * 4.2);
			var leadPos = GetLeadPosition(target, hitDelay);
			caster.TurnTowards(leadPos);
			var farPos = originPos.GetNearestPositionWithinDistance(leadPos, skill.Properties[PropertyName.MaxR]);
	...
	public class Mon_Cronewt_mage_Skill_1 : ITargetSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			...
			var originPos = caster.Position;
			var hitDelay = 700 + (int)(caster.Position.Get2DDistance(target.Position) * 2.9);
			var leadPos = GetLeadPosition(target, hitDelay);
			caster.TurnTowards(leadPos);
			var farPos = originPos.GetNearestPositionWithinDistance(leadPos, skill.Properties[PropertyName.MaxR]);
	...
	[SkillHandler(SkillId.Mon_Cronewt_Skill_1)]
	public class Mon_Cronewt_Skill_1 : ITargetSkillHandler
	{
		protected TimeSpan AniTime { get; } = TimeSpan.FromMilliseconds(700);
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			var hits = new List<SkillHitInfo>();
			await SkillAttack(caster, skill, splashArea, hitDelay, aniTime, hits);
			SkillResultTargetBuff(caster, skill, BuffId.UC_poison, 1, hits.Sum(h => h.HitInfo.Damage) * 0.2f, 12000f, 1, 15, -1, hits);
		}
	}
}
