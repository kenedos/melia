...
{
	[SkillHandler(SkillId.Mon_pampen_Skill_1)]
	public class Mon_pampen_Skill_1 : ITargetSkillHandler
	{
	...
	[SkillHandler(SkillId.Mon_pampen_Skill_2)]
	public class Mon_pampen_Skill_2 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			await EffectAndHit(skill, caster, position, new EffectHitConfig
			{
				...
				PositionDelay = 0,
				Effect = new EffectConfig("F_spread_out010_pink##1.2", 0.5f),
				Range = 30f,
				KnockdownPower = 100f,
				Delay = 0f,
				HitCount = 3,
