...
namespace Melia.Zone.Skills.Handlers.Mon
{
	...
	public class Mon_tala_combat_Skill_1 : ITargetSkillHandler
	{
		...
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			...
			var hitDelay = 600 + (int)(caster.Position.Get2DDistance(target.Position) * 3.1);
	...
	public class Mon_tala_combat_Skill_2 : ITargetSkillHandler
	{
		protected TimeSpan AniTime { get; } = TimeSpan.FromMilliseconds(800);
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			...
			var originPos = caster.Position;
			var hitDelay = 600 + (int)(caster.Position.Get2DDistance(target.Position) * 0.8);
			var leadPos = GetLeadPosition(target, hitDelay);
			caster.TurnTowards(leadPos);
			var farPos = originPos.GetNearestPositionWithinDistance(leadPos, skill.Properties[PropertyName.MaxR]);
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos, int hitDelay)
		{
			...
			splashParam = skill.GetSplashParameters(caster, originPos, farPos, length: 100, width: 10);
	...
	[SkillHandler(SkillId.Mon_tala_sorcerer_Skill_1)]
	public class Mon_tala_sorcerer_Skill_1 : ITargetSkillHandler
	{
		protected TimeSpan AniTime { get; } = TimeSpan.FromMilliseconds(800);
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
	...
	[SkillHandler(SkillId.Mon_tala_sorcerer_Skill_2)]
	public class Mon_tala_sorcerer_Skill_2 : ITargetSkillHandler
	{
		public void Handle(Skill skill, ICombatEntity caster, ICombatEntity target)
		{
			if (!caster.TrySpendSp(skill))
			...
			Debug.ShowPosition(caster.Map, farPos);
	...
	[SkillHandler(SkillId.Mon_tala_sorcerer_Skill_3)]
	public class Mon_tala_sorcerer_Skill_3 : ITargetSkillHandler
	{
		...
		private async Task HandleSkill(ICombatEntity caster, ICombatEntity target, Skill skill, Position originPos, Position farPos)
		{
			...
			if (!await MonsterCastTime(skill, caster, "Holy Ground", CastTimeMs, target))
				return;

			var targetPos = caster.Position.GetNearestPositionWithinDistance(target.Position, 150);
			SkillCreatePad(caster, skill, targetPos, 0f, PadName.Mon_Tala_Ground);
		}
	}
}
