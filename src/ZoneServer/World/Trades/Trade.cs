...
namespace Melia.Zone.World.Trades
{
	public class Trade
	{
		...
		private bool ValidateTrader(Character character, long worldId, int amount, out Item item, out Dictionary<long, int> traderItems, out Character trader)
		{
			...
			if (!trader.Inventory.TryGetItem(worldId, out item))
			{
		...
		private bool Finalized()
		{
			...
			{
				if (!trader1.Inventory.TryGetItem(offer.Key, out var item) || item.Amount < offer.Value || item.IsLocked)
				{
			...
			{
				if (!trader2.Inventory.TryGetItem(offer.Key, out var item) || item.Amount < offer.Value || item.IsLocked)
				{
			...
			{
				if (trader1.Inventory.TryGetItem(offer.Key, out var tradedItem))
				{
					trader1.Inventory.Remove(tradedItem, offer.Value, InventoryItemRemoveMsg.Given);
					var newItem = new Item(tradedItem, offer.Value);
			...
			{
				if (trader2.Inventory.TryGetItem(offer.Key, out var tradedItem))
				{
