...
namespace Melia.Zone.World
{
	public class PartyManager
	{
		...
		/// Excludes autotrading characters from the search.
		/// </summary>
			...
			lock (_parties)
			{
				foreach (var party in _parties.Values)
				{
					...
					foreach (var m in members)
					{
						...
						{
							// Skip autotrading characters - they should not be considered party members
							var character = ZoneServer.Instance.World.GetCharacter(c => c.DbId == m.DbId);
							if (character != null && character.IsAutoTrading)
								continue;
