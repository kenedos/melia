...
namespace Melia.Zone.World
{
	public class Party : Group
	{
		...
		public Party()
		{
			...
			this.ItemDistribution = (PartyItemDistribution)ZoneServer.Instance.Conf.World.PartyDefaultItemDistribution;
			this.ExpDistribution = (PartyExpDistribution)ZoneServer.Instance.Conf.World.PartyDefaultExpDistribution;
			this.QuestSharing = (PartyQuestSharing)ZoneServer.Instance.Conf.World.PartyDefaultQuestSharing;
		...
		/// </summary>
		/// <param name="character"></param>
		/// <returns>True if the member was removed, false if blocked.</returns>
		public bool RemoveMember(Character character)
		{
			if (this.TryGetMember(character.ObjectId, out var member))
			{
				// Block leaving if the party is in auto-match queue
				if (ZoneServer.Instance.World.AutoMatch.IsPartyInQueue(this))
				{
					character.ServerMessage("You cannot leave the party while in auto-match queue.");
					return false;
				}
		...
		private (long exp, long classExp) ApplyLevelPenalty(long baseExp, long baseClassExp, int characterLevel, int highestLevel)
		{
			...
			if (levelDifference >= conf.PartyExpLevelPenaltyFull)
			{
				penaltyMultiplier = 0.0f; // -100% exp
			}
			else if (levelDifference >= conf.PartyExpLevelPenaltyHigh)
			{
				penaltyMultiplier = 0.4f; // -60% exp
			}
			else if (levelDifference >= conf.PartyExpLevelPenaltyMedium)
			{
		...
		public void Expel(Character sender, string kickedPlayerTeamName)
		{
			...
			// Block kicking if the party is in auto-match queue
			if (ZoneServer.Instance.World.AutoMatch.IsPartyInQueue(this))
			{
				sender.ServerMessage("You cannot kick party members while in auto-match queue.");
				return;
			}

			// Block kicking if any party member is inside an instanced dungeon
			var partyMembers = this.GetPartyMembers();
			foreach (var partyMember in partyMembers)
			{
				if (partyMember != null && Scripting.DungeonScript.GetInstance(partyMember.DbId) != null)
				{
					sender.ServerMessage("You cannot kick party members while inside an instanced dungeon.");
					return;
				}
			}
