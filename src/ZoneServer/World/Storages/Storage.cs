...
namespace Melia.Zone.World.Storages
{
	...
	public abstract class Storage
	{
		...
		/// Properly handles sparse storage layouts by checking if there are
		/// any available positions within the storage size limit.
		/// </summary>
		/// <returns></returns>
		public bool CheckStorageFull()
		{
			lock (_syncLock)
			{
				if (_storageItems.Count < _storageSize)
					return false;

				// If we have exactly the storage size worth of items, check if they're
				// all within the valid position range (0 to _storageSize-1)
				// This handles sparse layouts where items might be at positions beyond
				// the storage size limit
				foreach (var position in _storageItems.Keys)
				{
					if (position >= _storageSize)
						return false; // Item is outside valid range, so not truly full
				}

				return true; // All positions are occupied within the valid range
			}
		}
