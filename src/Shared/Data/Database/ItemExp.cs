...
namespace Melia.Shared.Data.Database
{
	...
	public class ItemExpDb : DatabaseJson<ItemExpData>
	{
		...
		{
			var exp = this.Entries.Find(a => a.Group == group && a.Level == level)?.GainExp ?? 0;
		...
		public int GetNextExp(EquipExpGroup group, int level)
		{
			...
			var index = level - 1;
			var expList = this.Entries.Where(a => a.Group == group).OrderBy(a => a.Level).ToList();
			if (expList.Count == 0 || index < 0 || index >= expList.Count)
				return 0;
		...
		/// <summary>
		/// Returns the current level based on the accumulated exp value.
		/// Accumulates exp thresholds to determine the current level.
		/// </summary>
		/// <param name="group">The equipment group (e.g., "Gem")</param>
		/// <param name="exp">Current accumulated exp</param>
		/// <returns>Current level based on accumulated exp thresholds</returns>
		public int GetLevel(EquipExpGroup group, int exp)
		{
			...
			// If we've consumed all exp thresholds, return max level
			return entries.Max(a => a.Level);
		}
